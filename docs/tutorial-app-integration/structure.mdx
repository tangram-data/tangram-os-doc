...
A **resourceName** uniquely identifies each resource instance (like a database, table, or view) in your integration. The **resourceType** defines the resource category (e.g., Table), while the **resourceName** pinpoints which specific resource is referenced.

Tangram uses the **resourceNameTemplate** to extract the resourceName for each API operation, typically from path parameters or the request body.
...
```yaml
/api/v1/databases/{dbName}/tables/{tableName}:
  get:
    operationId: getIcebergTable
    parameters:
      - name: dbName
        in: path
        required: true
...
getIcebergTable {
  resourceType = "Table"
  resourceNameTemplate = "[{{parameters.dbName}}, {{parameters.tableName}}]"
  action = "Get"
}
```

Here, `resourceNameTemplate` tells Tangram to extract `dbName` and `tableName` from the path and use them to construct the resourceName.

For example, a request to `GET /api/v1/databases/demo/analytics/tables/click_events` would result in the following Resource Name (RN) URI:

```
rn://ai.tangram/iceberg/Table/demo/analytics/click_events
```

This format captures the resource type along with its hierarchical location, providing a globally unique identifier for the resource.
