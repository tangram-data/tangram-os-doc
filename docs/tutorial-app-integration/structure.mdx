...
This means:
- When Tangram calls `GET /api/v1/databases/{dbName}`, it's interpreted as a "Get" action on a "Database" resource with the name taken from the path parameter.
